...
function generateEventId(): string {
  return Math.random().toString(36).substr(2, 9) + '@navegate.us';
}
...
export function generateIcsContent(itinerary: TripItinerary): string {
  ...
  const icsContent = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//NaVegate//Travel Itinerary//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
  ...
  for (const day of itinerary.days) {
    for (const event of day.events) {
      ...
      if (event.venue) {
        ...
        description += `📍 ${escapeIcsText(event.venue.address)}`;
        if (event.venue.website) {
          description += `\\n🌐 ${escapeIcsText(event.venue.website)}`;
        }
        if (event.venue.safetyScore) {
          description += `\\n✅ Safety Score: ${event.venue.safetyScore}/100`;
        }
        description += '\\n\\n🌱 Generated by NaVegate';
      }
